...
def create_user(username: str, password: str, role: str = "user") -> int:
    ...
    with get_conn() as conn:
        cols = ["username", "password_hash", "password_salt", "role"]
        vals = [username, hash_hex, salt_hex, role]

        # include timestamps only if those columns exist
        if _has_col(conn, "users", "created_at"):
            cols.append("created_at"); vals.append(now)
        if _has_col(conn, "users", "updated_at"):
            cols.append("updated_at"); vals.append(now)

        placeholders = ", ".join("?" for _ in vals)
        q = f"INSERT INTO users({', '.join(cols)}) VALUES ({placeholders})"

        cur = conn.execute(q, vals)      # <- get the cursor
        uid = cur.lastrowid              # <- read lastrowid from the cursor
        conn.commit()
        return uid
...
def start_password_reset(username: str, *, ttl_minutes: int = 15) -> str | None:
    ...
    with get_conn() as c:
        ...
        # Be tolerant if reset columns don’t exist
        if _has_col(c, "users", "reset_code") and _has_col(c, "users", "reset_expires"):
            c.execute(
            ...
            # Columns missing: can’t store OTP; signal failure
            return None
...
def complete_password_reset(username: str, code: str, new_password: str) -> bool:
    ...
    with get_conn() as c:
        # If columns are missing, we can’t complete via OTP
        if not (_has_col(c, "users", "reset_code") and _has_col(c, "users", "reset_expires")):
            return False
        ...
        # If updated_at exists, update it too
        if _has_col(c, "users", "updated_at"):
            c.execute(
                "UPDATE users "
                "SET password_hash=?, password_salt=?, reset_code=NULL, reset_expires=NULL, updated_at=? "
                "WHERE id=?",
                (hash_hex, salt_hex, datetime.now().isoformat(timespec='seconds'), uid),
            )
        else:
            c.execute(
                "UPDATE users "
